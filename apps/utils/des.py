...
class DES(object):
    ...
    __K0 = partial(lambda k0, K2, k: map(K2,
                                         (k[k0[i]:28] + k[0:k0[i]] + k[k0[i] + 28:56] + k[28:k0[i] + 28] for i in
                                          range(16)))
        , __k0, __K2)
    __K = partial(lambda K1, K0, k: K0(K1(k))
        , __K1, __K0)

    def __init__(self, key, base=10):
        if base == 2:
            pass
        elif base == 16:
...
def encryt(raw_password, key):    
    '''对原始密码进行DES加密处理, key为密钥'''
    return DES(key).encode(raw_password)   

def decrypt(password, key):
    return DES(key).decode(password)
